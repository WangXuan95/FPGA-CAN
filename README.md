...
仿真顶层**tb_can_top.sv**描述了4个CAN总线设备互相进行通信的场景，每个设备都是一个**can_top.sv**的例化，**图1**是每个设备的详细属性，各个设备互相接收的关系可以画成左侧的箭头图，箭头代表了各个设备之间的接收关系。这些CAN设备的**本地ID**和**接收的ID**的配置方法详见详见[顶层模块说明](#顶层模块说明)，这里不多赘述。
